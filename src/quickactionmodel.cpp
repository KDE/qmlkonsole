...
{
    for (auto action : TerminalSettings::self()->actions())
...
{
    Q_EMIT beginInsertRows(QModelIndex(), m_actions.size(), m_actions.size());
    m_actions.push_back(action);
...
bool QuickActionModel::removeRows(int row, int count, const QModelIndex &parent)
{
    ...
    Q_EMIT beginRemoveRows(QModelIndex(), row, row + count - 1);
