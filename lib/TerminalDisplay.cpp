...
constexpr auto REPCHAR = "ABCDEFGHIJKLMNOPQRSTUVWXYZ" \
...
void TerminalDisplay::drawLineCharString(    QPainter& painter, int x, int y, QStringView str,
  ...
  for (y = 0; y < linesToUpdate; ++y)
  {
    ...
    if (updateLine)
    {
        ...
        QRect dirtyRect = QRect( _leftMargin+tLx ,
                                 _topMargin+tLy+qRound(_fontHeight)*y ,
  ...
  for (int y = luy; y <= rly; y++)
  {
    ...
    for (; x <= rlx; x++)
    {
      ...
      if ( _image[loc(x,y)].rendition & RE_EXTENDED_CHAR )
      {
        ...
        std::span chars = ExtendedCharTable::instance
  ...
  if (!_mouseMarks && !(ev->modifiers() & Qt::ShiftModifier))
  {
    ...
    if (ev->buttons() & Qt::RightButton)
        ...
        Q_EMIT mouseSignal( button,
                        charPos.columns + 1,
                        charPos.lines + 1 +_scrollBar->value() -_scrollBar->maximum(),
             1 );
...
void TerminalDisplay::extendSelection( const QPoint& position )
{
  ...
  QRect textBounds(tLx + _leftMargin,
                     tLy + _topMargin,
                   _usedColumns*qRound(_fontWidth)-1,
                   _usedLines*qRound(_fontHeight)-1);
  ...
  QPoint here = getCharacterPosition(pos); //QPoint((pos.x()-tLx-_leftMargin+(qRound(_fontWidth)/2))/qRound(_fontWidth),(pos.y()-tLy-_topMargin)/qRound(_fontHeight));
  QPoint ohere;
...
void TerminalDisplay::wheelEvent( QWheelEvent* ev )
{
  ...
  else
  {
    ...
    Q_EMIT mouseSignal( ev->angleDelta().y() > 0 ? 4 : 5,
                      pos.columns + 1,
                      pos.lines + 1 +_scrollBar->value() -_scrollBar->maximum() ,
                      0);
  }
}
...
void TerminalDisplay::setSize(int columns, int lines)
{
  ...
  QSize newSize = QSize( horizontalMargin + scrollBarWidth + (columns * _fontWidth)  ,
                 verticalMargin + (lines * qRound(_fontHeight))   );
...
//            m_session->removeView(this);
        ...
        connect(this, &TerminalDisplay::copyAvailable,
                m_session, &TerminalSession::selectionChanged);
        connect(this, &TerminalDisplay::termGetFocus,
                m_session, &TerminalSession::termGetFocus);
        connect(this, &TerminalDisplay::termLostFocus,
                m_session, &TerminalSession::termLostFocus);
        connect(this, &TerminalDisplay::keyPressedSignal,
                m_session, &TerminalSession::termKeyPressed);
...
void TerminalDisplay::simulateWheel(int x, int y, int buttons, int modifiers, QPoint angleDelta){
    ...
    QWheelEvent event(QPointF(x, y), mapToGlobal(QPointF(x, y)), pixelDelta, angleDelta,
                      (Qt::MouseButtons)buttons, (Qt::KeyboardModifiers)modifiers, Qt::ScrollBegin, false);
    wheelEvent(&event);
}
