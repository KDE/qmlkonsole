        ...
        , _foregroundPid(0)
{
    ...
    connect( _emulation.get(), &Emulation::titleChanged,
             this, &Session::setUserTitle );
    connect( _emulation.get(), &Emulation::stateSet,
             this, &Session::activityStateSet );
    connect( _emulation.get(), &Emulation::changeTabTextColorRequest,
             this, &Session::changeTabTextColorRequest );
    connect( _emulation.get(), &Emulation::profileChangeCommandReceived,
             this, &Session::profileChangeCommandReceived );

    connect(_emulation.get(), &Emulation::imageResizeRequest,
            this, &Session::onEmulationSizeChange);
    connect(_emulation.get(), &Emulation::imageSizeChanged,
            this, &Session::onViewSizeChange);
    connect(_emulation.get(), &Vt102Emulation::cursorChanged,
            this, &Session::cursorChanged);

    //connect teletype to emulation backend
    _shellProcess->setUtf8Mode(_emulation->utf8());

    connect( _shellProcess.get(), &Pty::receivedData,this,
             &Session::onReceiveBlock );
    connect( _emulation.get(),&Emulation::sendData, _shellProcess.get(),
             &Pty::sendData );
...
void Session::setView(TerminalDisplay * widget)
{
    ...
    if ( _emulation != nullptr ) {
        ...
        connect( widget , &TerminalDisplay::keyPressedSignal, _emulation.get() ,
                 &Emulation::sendKeyEvent);
        connect( widget , &TerminalDisplay::mouseSignal , _emulation.get() ,
                 &Emulation::sendMouseEvent );
        connect( widget , &TerminalDisplay::sendStringToEmu , _emulation.get() ,
                 [this](auto string) { _emulation->sendString(string); });

        // allow emulation to notify view when the foreground process
        // indicates whether or not it is interested in mouse signals
        connect( _emulation.get() , &Emulation::programUsesMouseChanged , widget ,
                 &TerminalDisplay::setUsesMouse );

        widget->setUsesMouse( _emulation->programUsesMouse() );

        connect( _emulation.get() , &Emulation::programBracketedPasteModeChanged ,
...
void Session::runEmptyPTY()
{
    ...
    disconnect( _emulation.get(),&Emulation::sendData,
                _shellProcess.get(), &Pty::sendData );
