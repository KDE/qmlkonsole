...
{
    return std::find_if(this->cbegin(), this->cend(), [filter](const auto &f) {
        return f.get() == filter;
    }) != this->cend();
}
...
void RegExpFilter::process()
{
    ...
    while(pos >= 0)
    {
        ...
        if ( pos >= 0 )
        {
            ...
            auto spot = newHotSpot(startLine,startColumn,
                                           endLine,endColumn);
...
std::unique_ptr<RegExpFilter::HotSpot> RegExpFilter::newHotSpot(int startLine,int startColumn,
                                                                int endLine,int endColumn)
{
    return std::make_unique<RegExpFilter::HotSpot>(startLine,startColumn,
                                                   endLine,endColumn);
}

std::unique_ptr<RegExpFilter::HotSpot> UrlFilter::newHotSpot(int startLine,int startColumn,int endLine,
                                                             int endColumn)
{
    auto spot = std::make_unique<UrlFilter::HotSpot>(startLine,startColumn,
                                                     endLine,endColumn);
