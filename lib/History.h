...
public:
 ...
 virtual ~HistoryScroll();
  ...
  virtual void getCells(int lineno, int colno, int count, std::span<Character> res) = 0;
  virtual bool isWrappedLine(int lineno) = 0;

  // backward compatibility (obsolete)
  Character   getCell(int lineno, int colno) { Character res; getCells(lineno,colno,1,std::span(&res, 1)); return res; }

  // adding lines.
  virtual void addCells(std::span<const Character> a, int count) = 0;
  // convenience method - this is virtual so that subclasses can take advantage
  // of QVector's implicit copying
  virtual void addCellsVector(const QVector<Character>& cells)
  {
    auto span = std::span<const Character> {cells.data(), size_t(cells.size())};
    addCells(span, cells.size());
  }
...
public:
  ...
  void getCells(int lineno, int colno, int count, std::span<Character> res) override;
...
public:
  ...
  virtual std::unique_ptr<HistoryScroll> scroll(std::unique_ptr<HistoryScroll> &&) const = 0;
};
...
public:
  ...
  std::unique_ptr<HistoryScroll> scroll(std::unique_ptr<HistoryScroll> &&) const override;
};
