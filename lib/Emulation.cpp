...
ScreenWindow* Emulation::createWindow()
{
    ...
    connect(window.get() , &ScreenWindow::selectionChanged,
            this , &Emulation::bufferedUpdate);

    connect(this , &Emulation::outputChanged,
            window.get() , &ScreenWindow::notifyOutputChanged );

    connect(this, &Emulation::handleCommandFromKeyboard,
            window.get(), &ScreenWindow::handleCommandFromKeyboard);
    connect(this, &Emulation::outputFromKeypressEvent,
            window.get(), &ScreenWindow::scrollToEnd);
...
void Emulation::setImageSize(int lines, int columns)
{
  ...
  QSize screenSize[2] = { QSize(_primaryScreen->getColumns(),
                                _primaryScreen->getLines()),
                          QSize(_alternateScreen->getColumns(),
                                _alternateScreen->getLines()) };
