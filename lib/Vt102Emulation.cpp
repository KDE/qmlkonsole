...
void Vt102Emulation::receiveChar(QChar character)
{
  ...
  if (getMode(MODE_Ansi))
  {
    ...
    if (lec(1,0,ESC+128)) { s[0] = ESC; receiveChar(u'['); return; }
...
void Vt102Emulation::processToken(int token, char16_t p, int q)
{
  switch (token)
  {
    ...
    case TY_CTL('X'      ) : _currentScreen->displayCharacter     (    QChar(0x2592)); break; //VT100
    case TY_CTL('Y'      ) : /* EM : ignored                      */ break;
    case TY_CTL('Z'      ) : _currentScreen->displayCharacter     (    QChar(0x2592)); break; //VT100
...
// change tab text color : \e[28;<color>t  color: 0-16,777,215
    ...
    case TY_CSI_PR('l', 1047) : _alternateScreen->clearEntireScreen(); resetMode(MODE_AppScreen); break; //XTERM
    ...
    case TY_CSI_PR('h', 1049) : saveCursor(); _alternateScreen->clearEntireScreen(); setMode(MODE_AppScreen); break; //XTERM
...
constexpr std::array<unsigned short, 32> vt100_graphics =
...
void Vt102Emulation::setMode(int m)
{
  ...
  switch (m)
  {
    ...
    case MODE_AppScreen : _alternateScreen->clearSelection();
                          setScreen(1);
    break;
  }
...
void Vt102Emulation::reportDecodingError()
{
  if (tokenBufferPos == 0 || ( tokenBufferPos == 1 && (tokenBuffer[0] & 0xff) >= 32) )
    return;
  qCDebug(qtermwidgetLogger) << "Undecodable sequence:" << QString::fromUtf16(tokenBuffer, tokenBufferPos);
}
